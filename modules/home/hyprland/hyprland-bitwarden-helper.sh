  ...
  case $1 in
    windowtitle*)
      ...
      if [[ "$window_title" == *"(Bitwarden Password Manager) - Bitwarden"* ]]; then
        ...
        hyprctl --batch "dispatch togglefloating address:0x$window_id ; dispatch resizewindowpixel exact 30% 60%,address:0x$window_id ; dispatch movewindowpixel exact 40% 20%,address:0x$window_id"
      fi
      ;;
  esac
}
