    ...
    services.hypridle = {
      ...
      settings = {
        ...
        in [
          ...
          # Screen lock
          (lib.mkIf cfg.settings.locking.enable { 
          ...
          (lib.mkIf cfg.settings.locking.dimming.enable { 
           ...
           # Screen off
          (lib.mkIf cfg.settings.screenOff.enable {
          ...
          # Hibernation AC
          (lib.mkIf cfg.settings.hibernation.enable {
            timeout = builtins.floor (60 * cfg.settings.hibernation.timeoutAC);
            on-timeout = "systemd-ac-power && systemctl hibernate";
          })

          # Hibernation battery
          (lib.mkIf cfg.settings.hibernation.enable {
            timeout = builtins.floor (60 * cfg.settings.hibernation.timeoutBattery);
            on-timeout = "systemd-ac-power || systemctl hibernate";
          })
        ];
      };
